...
if __name__ == '__main__':
    ...
    for N in [10,20,50,100,200,500,1000]:
        fig = plt.figure()
        title = title = r'ZZ, power law, $N = {}$'.format(N)
        plt.title(title)
        plt.xlabel(r'$t$')
        ...
        plt.tight_layout()
        plt.savefig('ZZ_dtwa/plots/variance_SN_vs_t_N_{}_all_ranges.png'.format(N))
        plt.close()
    ...
    # plt.tight_layout()
    # plt.savefig('ZZ_dtwa/plots/log_min_variance_SN_vs_range_all_N.png'.format(N))
    # plt.close()
    ...
    plt.ylim(bottom=None, top=None)
    plt.tight_layout()
    plt.savefig('ZZ_dtwa/plots/log_min_variance_SN_vs_log_N_all_ranges.png'.format(N))
    plt.close()

    def fn2(N, a, b, c, d):
        return a / (b + np.power(N, c)) + d
    fig = plt.figure()
    plt.title(r'ZZ, power law, $\min( N \cdot \langle {S_\alpha}^2 \rangle / {\langle S_x \rangle}^2 ) = A \cdot N ^ \beta$')
    plt.ylabel(r'$-\beta$')
    plt.xlabel(r'$\alpha$')
    range_list, b_list = [], []
    ...
    plt.ylim(0.001, 1)
    plt.tight_layout()
    plt.savefig('{}/plots/b_vs_range_for_min_variance_SN.png'.format('ZZ_dtwa'))
    plt.xscale('log')
    plt.yscale('log')
    plt.tight_layout()
    plt.savefig('{}/plots/log_b_vs_log_range_for_min_variance_SN.png'.format('ZZ_dtwa'))
    plt.close()
    ...
    fig = plt.figure()
    plt.title(r'ZZ, power law, $t_{opt} = A \cdot N ^ \mu$')
    plt.ylabel(r'$-\mu$')
    plt.xlabel(r'$\alpha$')
    range_list, m_list = [], []
    ...
    plt.legend(fontsize='x-small')
    plt.tight_layout()
    plt.savefig('{}/plots/m_vs_range_for_t_opt.png'.format('ZZ_dtwa'))
    plt.close()
