...
def powerLawJfunc_2d(j,alpha):
    Jfunc = lambda r: j/(r**alpha)
    return Jfunc
...
def getJij(coord,j,alpha,Jfunc=[]):
    if Jfunc==[] and coord.shape[1]==3: # if no interaction function provided, use 3D power law interaction 
        ...
        return J_ij 
    elif Jfunc==[] and coord.shape[1] in [1,2]: # if no interaction function provided, use 2D power law interaction 
        Jfunc = powerLawJfunc_2d(j,alpha)
...
def Heisenberg_A2A(L, Jperp, Jz, hx, hz, PauliBool = False, opList=[], Jfunc=[], coord = [], alpha=[]):
    ...
    if (alpha!=[] and alpha!=0) or Jfunc!=[]:          # ANISOTROPIC POWER LAW INTERACTIONS
        ...
        if len(Jfunc)!=3:
            ...
            Jx_ij = getJij(coord,Jx,alpha,Jfunc=[])
            Jy_ij = getJij(coord,Jy,alpha,Jfunc=[])
            Jz_ij = getJij(coord,Jz,alpha,Jfunc=[])
        else:
...
def getEDIsingSqueezing(N,H,initState,tvec, opList=[], PauliBool = False):
    ...
    squeezing_ed = (Vp_ed - np.sqrt(W_ed**2 + Vm_ed**2))/2

    # squeezing_ed = (Vp_ed - np.sqrt(W_ed**2 + Vm_ed**2))/(np.prod(N)/2)
    
    Sx_ed = np.array([expval_me(Qobj(initState),Sx_tot,H,t) for t in tvec])
    squeezing_SN_ed = squeezing_ed * N / (Sx_ed ** 2)

    if not np.allclose(squeezing_ed,squeezing_ed.real):
        raise Exception("Squeezing param imag for time range")
    
    return np.min(squeezing_ed.real), squeezing_ed.real, squeezing_SN_ed.real
...
def IsingSqueezing(N,Jz):
    ...
    squeezing_func = lambda t: (Vp_func(t) - np.sqrt(Vm_func(t)**2 + yz_acomm_func(t)**2))/2
