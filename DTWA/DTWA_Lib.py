...
def vec2config(vconfig):
    return np.reshape(vconfig,(int(len(vconfig)/3),3),order='F')

def XYderiv(vconfig, t, Jperp_ij):
    N = int(len(vconfig)/3)
    x = vconfig[0:N]
    y = vconfig[N:2*N]
    z = vconfig[2*N:3*N]
...
def Isingderiv(vconfig, t, J_ij):
    ...
    dxdt = -y*betaz
    dydt = x*betaz
    dzdt = np.zeros(np.shape(dydt))

    return np.concatenate((dxdt,dydt,dzdt),0)

def XXZderiv(vconfig, t, Jz_ij, Jperp_ij):
    N = int(len(vconfig)/3)
    x = vconfig[0:N]
    y = vconfig[N:2*N]
    z = vconfig[2*N:3*N]
...
def IsingEvolve_ODE(initConfigs,tvec,Jz,coord=[],Jfunc=[],alpha=[],Jij=[]):
    ...
    #     print("i: %d" %i)
        currtraj = odeint(Isingderiv,config2vec(initConfigs[i,:,:]),tvec,args=(J_ij,))
...
# initConfigs = initial configs in cartesian basis
def TFEvolve(initConfigs,tvec,h, axis='x'):
    config_evol = []
    for t in tvec:
        x0 = initConfigs[:,:,0]
        y0 = initConfigs[:,:,1]
        ...
        if axis == 'x':
            ...
            tconfig = np.stack((x0,yt,zt),axis=2)
        elif axis == 'y':
            xt = x0*np.cos(h*t) + z0*np.sin(h*t)
            zt = -x0*np.sin(h*t) + z0*np.cos(h*t)
            tconfig = np.stack((xt,y0,zt),axis=2)
        elif axis == 'z':
            yt = y0*np.cos(h*t) + x0*np.sin(h*t)
            xt = -y0*np.sin(h*t) + x0*np.cos(h*t)
            tconfig = np.stack((xt,yt,z0),axis=2)
